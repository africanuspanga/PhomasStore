...
class EcountApiService {
  ...
  private generateSupportEvidence(sessionInfo: any, inventoryTest: any): string {
    const zone = sessionInfo?.zone || 'IA';
    const baseUrlWithZone = this.baseUrl.replace('{ZONE}', zone);
    const endpoint = '/OAPI/V2/InventoryBalance/GetListInventoryBalanceStatus';
    const fullUrl = `${baseUrlWithZone}${endpoint}`;
    ...
    const lines = [
      ...
      '',
      '='.repeat(80),
      '🔧 TECHNICAL DETAILS (AS REQUESTED BY ECOUNT CS)',
      '='.repeat(80),
      '',
      '1. CALL METHOD:',
      '   POST',
      ...
      '   Accept: application/json',
      '   Accept-Language: en-US,en;q=0.9',
      '   Cookie: ECOUNT_SessionId={SESSION_ID}; SVID={SVID}',
      '',
      '5. DATA FORMAT (Request Body Structure):',
      '   {',
      '     "COM_CODE": "STRING(6) - Company Code",',
      ...
      '   }',
      '',
      '6. REQUEST VALUE (Actual Request Sent):',
      JSON.stringify(inventoryTest.requestBody || {}, null, 2),
      '',
      '7. RESULT VALUE (Actual Response Received):',
      ...
      `   TRACE_ID: ${inventoryTest.traceId || 'N/A (eCount does not return TRACE_ID in error response)'}`,
      '',
      '='.repeat(80),
      '',
      ...
      '',
      '❌ FAILING ENDPOINT:',
      `- Endpoint: ${inventoryTest.endpoint || endpoint}`,
      `- HTTP Status: ${inventoryTest.httpStatus || 'N/A'}`,
      `- Response Status: ${inventoryTest.responseStatus || 'N/A'}`,
      `- Error Message: ${inventoryTest.errorMessage || 'N/A'}`,
